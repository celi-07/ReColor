    ...
    <div className={`flex flex-col p-16 bg-turqoise`} id='section1'>
        ...
        <div className={`flex items-center flex-wrap ${width < 1200 ? 'justify-center' : 'justify-between'}`}>
          ...
          <div className={`flex flex-col flex-1`}>
            ...
            <p className={`font-focus text-white text-[18px] mb-8`}>We have done research to make the best out of food wastes and create the best color results. Our numerous number of dyes are the results of our research. Below are the most used food wastes in our products: </p>
            <InfoGraph
              text={'Fresh Vegetables'}
              barWidth={`${width < 500 ? 'w-[140px]' : width < 800 ? 'w-[280px]' :  'w-[560px]'}`}
              percent={'28'}
              fontSize={width < 500 ? 'text-[18px]' : 'text-[22px]'}
            />
            <InfoGraph
              text={'Drinks'}
            ...
            <InfoGraph
              ...
              fontSize={width < 500 ? 'text-[18px]' : 'text-[22px]'}
            />
            <p className={`font-focusItalic text-[18px] text-white mt-2`}>Source: ‘Food surplus and waste in the UK’ by Wrap</p>
          </div>
        </div>
        ...
        <div className={`flex items-end mt-16 flex-wrap ${width < 1140 ? 'justify-center' : 'gap-[5%]'}`}>
          ...
          <div className={`${width < 500 ? 'flex flex-col flex-wrap w-full break-words text-wrap' : 'w-[40%] min-w-[450px]'}`}>
            ...
            <p className={`font-focus text-[18px] text-white ${width < 1140 ? 'text-center' : 'text-right'}`}>Food waste is a pressing issue that not only contributes to environmental challenges like greenhouse gas emissions but also squanders valuable resources and nutrition. We can tackle this problem by creatively repurposing scraps into sustainable products, such as biodegradable natural dyes, while promoting mindful consumption practices that reduce waste at every stage of the food cycle.</p>
            <p className={`font-focusItalic text-[18px] text-white ${width < 1140 ? 'text-center' : 'text-right'} pt-4`}>Source: 2021 Food Waste Index by UN Environment Programme</p>
          </div>
        </div>
    </div>
  )
